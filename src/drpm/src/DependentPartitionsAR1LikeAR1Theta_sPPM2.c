...
* waic - 
*****************************************************************************************/
void debug_info(int i, int j, int t){
	Rprintf("iterate=%d  subject=%d  time=%d ###############\n",i,j,t);
}

void drpm_ar1_sppm_CODE(int *draws, int *burn, int *thin, int *nsubject, int *ntime,
...
//	Rprintf("csigETA1 = %f\n", csigETA1);
	...
	int iter_feedback = 1000;
	int verbose=0;
	Rprintf("keeping you updated every %d iterations\n", iter_feedback);
	for(i = 0; i < *draws; i++){
...
//						Rprintf("rho_comp = %d\n", rho_comp);
						...
						// if(rho_comp==0){
							// debug_info(i,j,t);
							// Rprintf("not compatible partitions:\n");
							// RprintIVecAsMat("comptm1",comptm1, 1, Rindx1+1);
							// RprintIVecAsMat("Si_red_1",Si_red_1, 1, Rindx1+1);
							// lgweight[k] = log(0);
						// }
...
//					Rprintf("denph = %f\n", denph);
					...
					// if (j==2 && i> (int)(*draws * 0.8)) {
						// Rprintf("working on subject j=%d\n",j);
						// Rprintf("nclusred = %d\n",nclus_red);
						// RprintVecAsMat("lgweight", lgweight, 1, nclus_red + 1);
					// }
...
//						Rprintf("Rindx1 = %d\n", Rindx1);
						...
						// RprintIVecAsMat("Si_iter", Si_iter, *nsubject, ntime1);
						// RprintIVecAsMat("gamma_iter", gamma_iter, *nsubject, ntime1);
						// Rprintf("non compatible partition occurred when checking for gamma=0->1.\n");
						// debug_info(i,j,t);	
						// RprintIVecAsMat("comptm1", comptm1, 1, *nsubject);
						// RprintIVecAsMat("comp1t", comp1t, 1, *nsubject);
							probh[1] = 0;
...
// 				Rprintf("current unit j=%d and time t=%d\n", j,t);
// 				Rprintf("gamma_iter[j*(ntime1) + t] = %d\n", gamma_iter[j*(ntime1) + t]);
// 				Rprintf("nclus_red = %d\n", nclus_red);
// 				Rprintf("n_red = %d\n", n_red);
// //				RprintIVecAsMat("nh_red = ", nh_red, 1, *nsubject);
...
//						RprintIVecAsMat("comptp1", comptp1, 1, *nsubject);
						...
						if(rho_comp != 1){
							// Rprintf("non compatible partition occured when checking for rho.\n");
							// debug_info(i,j,t);
							// Rprintf("non compatible partition occurred at line 839.\n");
							ph[k] = log(0); // Not compatible	
						// RprintIVecAsMat("gamma_iter", gamma_iter, *nsubject, ntime1);
						// RprintIVecAsMat("Si_iter", Si_iter, *nsubject, ntime1);
						// Rprintf("non compatible partition occurred when imposing unit j in cl k(+1)=%d.\n",k+1);
						// debug_info(i,j,t);	
						// RprintIVecAsMat("comp2t", comp2t, 1, *nsubject);
						// RprintIVecAsMat("comptp1", comptp1, 1, *nsubject);				
...
//			RprintIVecAsMat("nclus_iter", nclus_iter, 1, ntime1);
			...
			relabel(Si_tmp, *nsubject, Si_tmp2, reorder, oldLab);	
			// (testing if useless)	
			// yes it appears to be useless, no problems raised
			// Rprintf("###### SECOND RELABEL iter:%d time:%d ####### \r",i,t);
...
/* COMMENT
Within a single time point t, the cluster assignments and parameters are repeatedly updated and overwritten as each subject is processed.

Forse perché giustamente il loop più esterno è sulle mcmc iterates, quindi dentro al codice effettivo che fa il lavoro si lavora sempre sul "creare un singolo sample, una singola draw", la i corrente. Quindi questo lo si fa processando le unità e sovrascrivendo sempre il risultato fino a "convergere" (percorrendo i soggetti e il tempo) al risultato finale (per la i-esima iterata)
*/
