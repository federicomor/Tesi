...
for (unit in 1:N){
	...
	plot(drpm1$eta1[,unit],type="l",
		 main=bquote("Model DRPM C\nTrace plot of tau2 at unit "*.(unit)),
		 xlab = "MCMC iterations",ylab="values")
...
# cols=colora(len=10, seed=67, show=0)
cols=colora(len=10, seed=44, show=0)
...
# niter=40000; nburn=22000; nthin=10 # folder section 2 run 40k
nout <- (niter-nburn)/nthin
cat(nout,"valid iterations\n")
```
...
```{r}
# save(drpm1, rout, out, file = "ran_section2_withspace_30k22b8t.Rdata")
load(file = "ran_section2_withspace_30k22b8t.Rdata")
```
...
for (t in 1:Tm){
	cat("Time",time,"- #clusters =",length(unique(partitions_drpmc[[t]])),"\n")
}
cat("\n")
cat(crayon::red("DRPM - J\n"))
for (t in 1:Tm){
	cat("Time",time,"- #clusters =",length(unique(partitions_drpmj[[t]])),"\n")
}
```


```{r, warning=F}
cols=colora(len=maxNClusters, seed=67, show=0) # purples
# cols=colora(len=10, seed=44, show=0) # greens
# cols = colora(len=10,"rand")
...
# Plot lines and points for each unit
for (unit in 1:num_units) {
  lines(1:Tm, partition_matrix[unit, ]+unit/100, type = "l", pch = 19,
        col = unit, lwd = 1)
}
legend("topright", legend = paste("Unit", 1:num_units), col = 1:num_units,
       lty = 1, lwd = 2, pch = 19, bty = "n")
...
colnames(df_wsc)
covariate_scelte = c("Altitude","WE_tot_precipitation",
					 "EM_nox_traffic","WE_wind_speed_100m_mean")
for(i in 1:n){
	st = stations[i]
	for(t in 1:Tm){
...
Elapsed time: 2 seconds, 753 milliseconds
LPML: -52.09245933075181 (the higher the better)
WAIC: 10.256530969537543 (the lower the better)
...
```{r}
save(drpm1, rout, out, file = "ran_30k_with_space.Rdata")
# load(file = "ran_space_data.Rdata")
```
...
for (time in 1:Tm){
	par(mfrow=c(1,2),mar=c(2,2,4,2))
	plot(drpm1$theta[,time],type="l",
		  ylim=c(min(drpm1$theta[,unit],rout$theta[,unit]),
		 	    max(drpm1$theta[,unit],rout$theta[,unit])),
		 main=bquote("Model DRPM C\nTrace plot of theta at time "*.(time)),
		 xlab = "MCMC iterations",ylab="values")
	plot(rout$theta[,time],type="l",
		  ylim=c(min(drpm1$theta[,unit],rout$theta[,unit]),
...
plot(drpm1$lam2[,1],type="l",
	 	  ylim=c(min(drpm1$lam2[,1],rout$lambda2[,1]),
		 	    max(drpm1$lamb2[,1],rout$lambda2[,1])),
	 main=bquote("Model DRPM C\nTrace plot of lambda2"),
	 xlab = "MCMC iterations",ylab="values")
plot(rout$lambda2[,1],type="l",
	 	  ylim=c(min(drpm1$lam2[,1],rout$lambda2[,1]),
