...
```{r}
numerical_covariates_focus = c("WE_temp_2m","Altitude","WE_wind_speed_10m_mean")
...
out = module_JDRPM$MCMC_fit(
	...
	# covariate_similarity_idx = NA,  
	draws = niter, burnin = nburn, thin = nthin,
	# draws = 60000, burnin = 55000, thin = 5,
	logging = TRUE,
	seed = seed
)
```
...
for(i in 1:size(yredJ)[1]){
   if(i==1){
     plot(1:size(yredJ)[2],yredJ[i,],col=cols[i],
     	 ...
     	 type='l',xlab='time',ylab="fitted values",main="model J - no Xlk")
 	  } 
	  else{
		  lines(1:size(yredJ)[2],yredJ[i,],col=cols[i])
	  }
...
for(i in 1:size(yredJ)[1]){
   if(i==1){
     plot(1:size(yredJ)[2],yredJ[i,],col=cols[i],
     	 ylim=extrema(yred,yredJ,yredJ),
     	 type='l',xlab='time',ylab="fitted values",main="model J - with Xlk")
 	  } 
	  else{
		  lines(1:size(yredJ)[2],yredJ[i,],col=cols[i])
	  }
...
cols=colora(len=maxNClusters, seed=56, show=0) # la 56
# cols = colora(len=10,"rand")
