...
time_span = 1:3 # low time span for quick testing, real one will be 1:53
# nsubjects = 1:105
y_fit = y[,1+time_span]
...
# niter=50000; nburn=10000; nthin=40 # they did this in their tests
# niter=60000; nburn=20000; nthin=20
niter=12000; nburn=6000; nthin=4
nout <- (niter-nburn)/nthin
cat(nout,"valid iterations\n")
```
...
for (time in 1:Tm){
	...
	plot(drpm1$mu[time,unitj,],type="l",
		 ylim=c(min(drpm1$mu[time,unitj,],rout$mu[time,unitj,]),
		 	   max(drpm1$mu[time,unitj,],rout$mu[time,unitj,])),
		 main=bquote("Model DRPM C\nTrace plot of mu at time "*.(time) * " unit " * .(unitj)),
		 xlab = "MCMC iterations",ylab="values")
	plot(rout$muh[time,unitj,],type="l",
		 ylim=c(min(drpm1$muh[time,unitj,],rout$muh[time,unitj,]),
		 	   max(drpm1$muh[time,unitj,],rout$muh[time,unitj,])),
		 main=bquote("Model DRPM J\nTrace plot of mu at time "*.(time) * " unit " * .(unitj)),
		 xlab = "MCMC iterations",ylab="values")
}
```
