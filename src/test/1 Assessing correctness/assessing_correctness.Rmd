...
```
```{r}
# save(rout,file="no_space_20k_rout_full.Rdata")
```
...
plot_partition_with_numbers = function(partition,title,cex=1.5){
...
if (save_plots==T){
	pdf(file="ari.pdf",width=12,height = 6)
}
...
if (save_plots==T){
	pdf(file="partizioni_nums.pdf",width=12,height = 6)
}
...
for(i in 1:size(yred)[1]){
   if(i==1){
     plot(1:size(yred)[2],yred[i,],col=cols[i],
     	 ylim=extrema(as.matrix(10*yfull[,-c(1)])),
...
# niter=120000; nburn=80000; nthin=40
...
# save(drpm1,file="space_100k_drpm1.Rdata")
```
...
out = module_JDRPM$MCMC_fit(
	...
	# covariate_similarity_idx = NA,
	# draws = as.integer(10),burnin = as.integer(0), thin = as.integer(1),
...
```{r}
save(drpm1,rout,file="test_space_80k_n105_T16.Rdata")
```
...
generate_partition = function(model,maxncl=0){
	...
	for (t in 1:Tm){
		...
		Si_jt <- t(Si_jt) 
		partition[[t]] <- salso(Si_jt, loss = "binder",
		# partition[[t]] <- salso(Si_jt, loss = binder(a=1.2),
								maxNClusters = maxncl)
	}
	return(partition)
}

plot_partition_with_numbers = function(partition,title,cex=0.5){
	partition_matrix <- do.call(cbind, partition)
	plot(NULL, NULL, xlim = c(1, Tm), ylim = c(1, N)+c(-0.5,0.5),
	     xlab = "time", ylab = "units",
	     xaxt = "n", yaxt = "n", main = title)
...
par(mar=c(4,1,3,1),mfrow=c(1,2),oma=c(0.5,0.1,0.1,0.1))
plot_partition_with_numbers(partitions_drpmc, title="model C",cex=0.6)
plot_partition_with_numbers(partitions_drpmj, title="model J",cex=0.6)
if (save_plots==T){ dev.off()}
...
for(t in 1:Tm){
	...
	plot(s_std[1,1],s_std[1,2],col=partition_matrix[1,t],pch=19,
		 xlim=extrema(s_std[,1]),main=paste("J - time",t),
	...
	partition_matrix <- do.call(cbind, partition)
	plot(s_std[1,1],s_std[1,2],col=partition_matrix[1,t],pch=19,
		 xlim=extrema(s_std[,1]),main=paste("C -time",t),
		 ylim=extrema(s_std[,2]),
		 cex=((y[1,t]-mean(y[,t]))/sd(y[,t])+3)/2)
		 # cex=y[1,t]+abs(min(y[,t])))
	for (i in 1:N){
