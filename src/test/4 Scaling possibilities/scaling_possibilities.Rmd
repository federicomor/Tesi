...
brks <- seq(0,max(performance_C,performance_Julia)*1000, length.out = ncols_ari + 1)
...
for (i in 1:nrow(mat)) {
  for (j in 1:ncol(mat)) {
    text(x = j - 0.9, 
    	 y = i - 0.9, 
    	 labels = sprintf("%.2f ms/it", mat[i, j]*1000), cex = 1.0, col = "#aaaaaa")
  }
}
...
plot_mat(performance_C,title="C matrix")
plot_mat(performance_Julia,title="J matrix")
```
