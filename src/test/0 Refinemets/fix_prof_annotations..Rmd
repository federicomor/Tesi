...
abline(h=0,col="#dCd5d2")
	for (time in 1:12){
		...
		# points(time,LOW,cex=1,col="#F65A6D",pch=19)
		dx = 0.1
		lines(c(time,time),c(LOW,UPP),col="#FFD99F",lwd=1.4)
		lines(c(time-dx,time+dx),rep(MID,2),lwd=1.7,col="#7D1D67")
		lines(c(time-dx,time+dx),rep(UPP,2),lwd=1.7,col="#F65A6D")
		lines(c(time-dx,time+dx),rep(LOW,2),lwd=1.7,col="#F65A6D")
	}
}
