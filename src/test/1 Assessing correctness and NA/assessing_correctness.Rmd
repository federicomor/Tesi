...
for (i in sorted_indexes){
	...
	sampled_values = rout$fitted[t,j,]
		...
		MID = mean(sampled_values)
		LOW = ci_hdi[[2]]
		lines(c(idx,idx),c(LOW,UPP),col="#FFD99F",lwd=4)
		lines(c(idx-dx,idx+dx),rep(MID,2),cex=1,col="#7D1D67",pch=15,lwd=4)
		lines(c(idx-dx,idx+dx),rep(UPP,2),cex=1,col="#F65A6D",pch=19,lwd=4)
		lines(c(idx-dx,idx+dx),rep(LOW,2),cex=1,col="#F65A6D",pch=19,lwd=4)
...
for (i in sorted_indexes){
	...
	sampled_values = rout$fitted[t,j,]
		...
		if (yred[j,t]<=UPP && yred[j,t]>=LOW){
		lines(c(idx,idx),c(LOW,UPP),col="#FFD99F",lwd=2)
		lines(c(idx-dx,idx+dx),rep(MID,2),cex=1,col="#7D1D67",pch=15,lwd=2)
