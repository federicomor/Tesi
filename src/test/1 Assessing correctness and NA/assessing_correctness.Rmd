...
if (save_plot==T) { pdf(file=paste0(folder,target,".pdf"),width = 7,height = 4)}
par(mar=c(2,2,2,1))
col_idx = 1
for(i in ranked_indices){
   if(i==ranked_indices[1]){
...
save(rout,file="space_rout_full_run6_dfweekly_backup.Rdata")
...
# LOAD models
```{r}
load("./df wsc fits/space_drpm1_run5.Rdata")
...
load("df wsc fits/space_drpm1_run5.Rdata") # final choice since really same model
...
#################################################################
if (save_plots==TRUE) { 
	pdf(file="J_mean_prediction_NApluslk.pdf",height = 6,width=11)
	# pdf(file="J_median_prediction.pdf",height = 6,width=10)
	# pdf(file="J_last_iteration.pdf",height = 6,width=10)
}
summary_fitted_NA = t(rout_NA$fitted[,,size(rout_NA$fitted)[3]])*0
