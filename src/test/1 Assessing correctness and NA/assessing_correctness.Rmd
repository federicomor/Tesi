...
save(drpm1,file="space_drpm1_run1.Rdata")
```
...
a_tau    = 1.7; b_tau   = 0.4
# a_lambda = 0.1; b_lambda = 0.1
# a_lambda = 1.7; b_lambda = 0.4
a_lambda = 4.1; b_lambda = 0.4
...
save(rout,file="space_rout_full_run6_dfdaily.Rdata")
# load("space_rout_run5.Rdata")
```
...
# load model
```{r}
load("space_rout_full_run2_dfdaily.Rdata")
```
...
```{r}
# load("space_rout_full_run1_dfdaily.Rdata")
# load("space_full_100k_run2.Rdata")
```
...
par(mar=c(3.2,1,2,1),mfrow=c(1,2),oma=c(0.1,0.1,0.1,0.1))
plot_partition_with_numbers(partitions_drpmc, title="model C",cex=0.9,line=2.1)
plot_partition_with_numbers(partitions_drpmj, title="model J",cex=0.9,line=2.1)
if (save_plots==T){ dev.off()}
```
