...
y_other <- t(t(y_other) - mn)

Tm = tps <- ncol(y_other) # time span
N = size(y_other)[1] # number of units
num_units = N
...
if (save_plot==T) { pdf(file=paste0(folder,target,".pdf"),height = 5.9,width = 11)}
par(mar=c(2,2,2,1))
col_idx = 1
for(i in ranked_indices){
   if(i==ranked_indices[1]){
...
# vals = as.numeric(df_wsc[df_wsc$Time==times[11],"EM_nh3_sum"][[1]])
vals = as.numeric(df_wsc[df_wsc$Time==times[11],"WE_tot_precipitation"][[1]])
# vals = as.numeric(df_wsc[df_wsc$Time==times[11],"WE_blh_layer_max"][[1]])
# vals = as.numeric(df_wsc[df_wsc$Time==times[11],"WE_wind_speed_10m_max"][[1]])
# vals = as.numeric(df_wsc[df_wsc$Time==times[11],"WE_wind_speed_100m_max"][[1]])
mv = mean(vals)
vals - mv
```
...
```{r}
save(rout,file="J_run18_as11_but_with_higher_cvweight.Rdata")
```
...
load("../1 Assessing correctness and NA/df wsc fits/space_drpm1_run5.Rdata")
```
...
# pdf(file="trace_plots_with_Xlk.pdf",height = 6,width=11)

size(rout$fitted)
par(mfrow=c(2,1),mar=c(1.5,2,2,1),oma=c(0.8,0.1,0,0.1))
plot(rout$fitted[t,j,],type="l",main=paste0("J (with Xlk) trace plot of unit ",j," at t=",t),
	 ylim=extrema(y[j,t],rout$fitted[t,j,],drpm1$fitted[t,j,]))
abline(h=y[j,t],col="#005500")
plot(drpm1$fitted[t,j,],type="l",main=paste0("C trace plot of unit ",j," at t=",t),
	 ylim=extrema(y[j,t],rout$fitted[t,j,],drpm1$fitted[t,j,]))
abline(h=y[j,t],col="#005500")
...
folder = "run 4 images/"
name = "JDRPM target + space + Xcl (p=4)"
partition = partitions_drpmj
```
...
for (target in covariate_scelte){
	dir.create(paste0(folder,target))
for (time in 1:Tm){
##### otherwise
if (save_plot==T) {
	pdf(paste0(folder,target,"/",name,"_t",time,".pdf"),width = 9.9, height = 5.4)
}
par(mar=c(2,4,2,1),oma=c(0.1,0.1,0.1,0.1))

cova_of_interest = as.data.frame(df_wsc[df_wsc$Time == unique(df_wsc$Time)[time],target])
...
# ===========
# ari two cols comparison
```{r,warning=F}
name_sp = "JDRPM target + space"
load("../1 Assessing correctness and NA/df wsc fits/space_rout_full_run6_dfweekly.Rdata")
partition_sp = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL_sp = rout

name_Xcl = "JDRPM target + space + Xcl"
load("J_space_run11_Xcl_sim4_wspmax10m_totprec_bhlmax.Rdata")
partition_Xcl = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL_Xcl = rout
```
...
save_plots = T
if (save_plots==T){
	pdf(file="ari_sp_vs_Xcl.pdf",width=10,height = 5)
}
par(mar=c(2,2,2,2),mfrow=c(1,2),oma=c(0.1,0.1,0.1,2))
plot_ARI(partition_sp, title=name_sp)
plot_ARI(partition_Xcl, title=name_Xcl)
if (save_plots==T){ dev.off()}
```
...
name = "JDRPM target + space"
load("../1 Assessing correctness and NA/df wsc fits/space_rout_full_run6_dfweekly.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
```{r,warning=F}
folder = "run 3 images/"
name = "JDRPM target + space + Xcl (p=3)"
load("J_space_run3_Xcl_Altitude_wsp10mmax_totprec.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
folder = "run 4 images/"
name = "JDRPM target + space + Xcl (p=4)"
load("J_space_run4_Xcl_wsp10mmax_totprec_rhmean_bhlmax.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
folder = "run 7 images/"
name = "JDRPM target + space + Xcl (p=3)"
load("J_space_run7_Xcl_sim4_wsp10mmax_totprec_bhlmax.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
folder = "run 8 images/"
name = "JDRPM target + space + Xcl (p=3)"
load("J_space_run8_Xcl_sim4_highercvweight_wsp10mmax_totprec_bhlmax.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
```{r,warning=F}
folder = "spaceJ Xcl Xlk fit13/"
name = "JDRPM target + Xcl + Xlk"
load("J_run13_Xcl_sim1_Xlk.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
```{r,warning=F}
folder = "spaceJ Xcl fit16/"
name = "JDRPM target + space + Xcl"
load("J_run16_as11_but_with_also_rhmean.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```

```{r,warning=F}
folder = "spaceJ Xcl fit18/"
name = "JDRPM target + space + Xcl"
load("J_run18_as11_but_with_higher_cvweight.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```

```{r,warning=F}
folder = "spaceJ Xcl fit larger cv weight/"
name = "JDRPM target + space + Xcl"
load("J_run17_as11_but_with_also_altitude_and_higher_cvweight.Rdata")
partition = partitions_drpmj = generate_partition(rout, loss ="binder",maxcl = 0)
MODEL = rout
```
...
############################################## ARI
if (save_plot==T){
	pdf(file=paste0(folder,"ari",name,".pdf"),width=6,height = 5)
}
# par(mar=c(2,2,2,2),mfrow=c(1,2),oma=c(0.1,0.1,0.1,2))
par(mar=c(2,2,2,2),oma=c(0.1,0.1,0.1,0.1))
plot_ARI(partition, title=name)
if (save_plot==T){ dev.off()}
...
if (save_plots==TRUE) { 
	pdf(file=paste0(folder,"fitted_values",name,".pdf"),height = 6,width=11)
}
...
for(i in 1:size(yred)[1]){
   if(i==1){
     plot(1:size(yred)[2],yred[i,],col=cols[i],
     	 ylim=extrema(yredJ,y),type='l',xlab='',ylab="",
     	 # main="J fitted values - last iteration")
     	 main=paste0(name, " - fitted values - iterations mean"))
     	 # main="J fitted values - iterations median")
 	  } 
	  else{
		  lines(1:size(yred)[2],yred[i,],col=cols[i])
...
summary_fitted_mean = t(MODEL$fitted[,,size(MODEL$fitted)[3]])*0
summary_fitted_median = t(MODEL$fitted[,,size(MODEL$fitted)[3]])*0
for(j in 1:N){
	for(t in 1:Tm){
		summary_fitted_mean[j,t] = mean(MODEL$fitted[t,j,])
		summary_fitted_median[j,t] = median(MODEL$fitted[t,j,])
	}
}
cat(name,"\nMSE\n")
cat("mean ", MSE_matrices(summary_fitted_mean,yredORIGI),"\n")
...
```
```{r}
cls = partitions_drpmj[[1]]
cls
maxk = max(cls)
idx_sorted = c()
lens = rep(0,maxk)
for (k in 1:maxk){
	idx_sorted = c(idx_sorted,which(cls == k))
	lens[k] = length(which(cls==k))
}
idx_sorted
lens
sum(lens)
colors = c()
for (k in 1:maxk){
	colors = c(colors,rep(k,lens[k]))
}
colors

cova_of_interest = as.data.frame(df_wsc[df_wsc$Time == unique(df_wsc$Time)[time],target])[[1]]

plot(NA, xlim = c(1, 105), ylim = extrema(cova_of_interest), 
     xlab = "", ylab ="",
     main = "time t")
for (i in 1:105){
	points(i,cova_of_interest[idx_sorted[i]],col=colors[i],pch=19)
}
```
...
if (save_plot==T) {
	# pdf(paste0(folder,target,"/",name,"_t",time,".pdf"),width = 9.9, height = 5.4)
	pdf(paste0(folder,"/",target,"_sorted.pdf"),width = 9.9, height = 5.4)
}

par(mar=c(2,2,2,1),mfrow=c(3,4),oma=c(0.2,0.1,1.5,0.1))
for (time in 1:Tm){
cova_of_interest = as.data.frame(df_wsc[df_wsc$Time == unique(df_wsc$Time)[time],target])[[1]]
...
plot(NA, xlim = c(1, 105), ylim = extrema(cova_of_interest), 
     xlab = "", ylab ="", main = paste0("time ", time))
for (i in 1:105){
	points(i,cova_of_interest[idx_sorted[i]],col=colors[i],pch=19,cex=1.3)
}


}
mtext(paste0(name," - ",target, " (sorted by cluster)"),side=3,line=0,outer=T,col="#2E1831")
if (save_plots==TRUE) { dev.off() }
}

```

## FOCUS covariate sorted
```{r,warning=F}
save_plot = save_plots = F
...
if (save_plot==T) {
	# pdf(paste0(folder,target,"/",name,"_t",time,".pdf"),width = 9.9, height = 5.4)
	# pdf(paste0(folder,"/",target,"_sorted.pdf"),width = 9.9, height = 5.4)
	# pdf(paste0(target,"_CDRPM_sorted_ZOOM.pdf"),width = 8, height = 4)
	pdf(paste0(target,"_JDRPM_sorted_ZOOM.pdf"),width = 8, height = 4)
}

par(mar=c(2,2,2,1),mfrow=c(1,2),oma=c(0.2,0.1,1.5,0.1))
for (time in c(9,12)){
cova_of_interest = as.data.frame(df_wsc[df_wsc$Time == unique(df_wsc$Time)[time],target])[[1]]
...
if (save_plot==T) {
	# pdf(paste0(folder,target,"/",name,"_t",time,".pdf"),width = 9.9, height = 5.4)
	pdf(paste0(folder,"/",target,".pdf"),width = 9.9, height = 5.4)
}
...
mtext(paste0(name," - ",target),side=3,line=0,outer=T,col="#2E1831")
if (save_plots==TRUE) { dev.off() }
}
...
##########################
df_cluster = data.frame(Longitude=c(),Latitude=c(),values=c(),clusters=c(),Time=c())
for(time in time_span[limt_start:limt]){
...
covariata = covariata,annotate = F,global_ylim = F)
...
ggsave(file=paste0(folder,"layout_small/",model_name,"_t",cur_num,"_YLIMS.pdf"),plot = g,
...
if (save_plot==TRUE){
	dir.create(paste0(folder,"layout_large"))

ggsave(file=paste0(folder,"layout_large/",model_name,"_t",cur_num,".pdf"),plot = g,
	   # units="px",width=2500, height=1200, dpi=300)
	   # units="px",width=4500, height=2000, dpi=300)
	   # units="px",width=3600, height=1500, dpi=300)
...
# Generate random data with smaller spread
n_samples <- 150
n_clusters <- 3
cluster_spread <- 2  # Smaller spread for mixed clusters
data <- data.frame(
  x = rnorm(n_samples) / cluster_spread,
  y = rnorm(n_samples) / cluster_spread
)

# Add cluster centers closer to each other
centers <- matrix(runif(n_clusters * 2, min = -2, max = 2), ncol = 2)
data$x <- data$x + rep(centers[,1], each = n_samples / n_clusters)
data$y <- data$y + rep(centers[,2], each = n_samples / n_clusters)

# Perform KMeans clustering
clusters <- kmeans(data, centers = 3)$cluster

pdf(file="example_clusters.pdf",width=9,height=4)

# Plotting with base R
par(mfrow = c(1, 2),mar=c(2,2,2,1)) # Set up side-by-side plots

# Unclustered data
plot(data$x, data$y, col = "gray", pch = 19, main = "random data",
     xlab = "", ylab = "")

# Clustered data
plot(data$x, data$y, col = clusters, pch = 19, main = "clustered data",
     xlab = "", ylab = "")
